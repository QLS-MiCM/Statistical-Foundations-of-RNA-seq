...
y <- rnbinom(20, mu = lambda, size = 100)
y_log <- log(y+0.000001)

# Fit three models
lm_fit   <- lm(y_log ~ x)                        # Gaussian linear model
...
Here the two tsv files should be in the same directory as this .Rmd file. They can be downloaded from Github in the Exercises/Data folder.
...
PCA before filter/normalization

```{r}
# turn gene counts into dataframe
pca_genes <- prcomp(t(dge$counts))
d_pca <- as.data.frame(pca_genes$x) 

# create PCA plot
pca_plot <- ggplot(d_pca, aes(x = PC1,y = PC2, color = group)) +
  labs(title = "Principle Component Analysis on ADvsCt RNA-seq Data") +
...
```{r}
before_n_genes <- nrow(dge)
# optional: ribosomal RNA filter from geneID list
# Minimum gene expression filter --> keep genes with at least 5 read/million in at least n samples
data_cpm <- cpm(dge)               # calculate counts per million
keep <- rowSums(data_cpm >= 1) >= 4      # creates boolean vector
dge <- dge[keep,]             # filter data for genes that pass threshold
after_n_genes <- nrow(dge)

# Display filtering results
n_genes_gone <- before_n_genes - after_n_genes
cat(n_genes_gone, "genes filtered out.\n", "Before: ", before_n_genes, "\n", "After: ", after_n_genes)

# normalize for library size with CPM
cpm_counts <- cpm(dge)
# edgeR takes raw count since GLM fitting accounts for normalization!
#dge$counts <- cpm_counts

# further normalization with TMM
...
PCA after filter/normalization
...
Log-Count to perform PCA

```{r}
logCPM <- cpm(dge, log = TRUE, prior.count = 2)

pca_genes <- prcomp(t(logCPM), scale. = TRUE)
d_pca <- as.data.frame(pca_genes$x)
d_pca$group <- group

ggplot(d_pca, aes(PC1, PC2, color = group)) +
  geom_point(size = 3) +
  theme_classic() +
  labs(title = "PCA on TMM-normalized logCPM")
```
...
design <- model.matrix(~ group) # specify the design model
...
res <- topTags(lrt_nb, n = Inf)$table
```
...
ggplot(res, aes(x = logFC, y = -log10(FDR), color = significant)) +
  ...
  theme_classic() +
  labs(
    title = "AD vs Control differential expression",
    x = "log2 fold change",
    y = "-log10 FDR"
  )
```
...
```{r}
plotMD(lrt_nb, status=decideTests(lrt_nb),
       main="MA Plot (LRT with Dispersion)", ylim=c(-20,20))

# Add labels manually
sig_idx <- which(res$significant)

text(
  x = res$logCPM[sig_idx],
  y = res$logFC[sig_idx]+10,
  labels = res$Symbol[sig_idx],
  pos = 3,
  cex = 0.7
)
```
